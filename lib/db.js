...
export async function semayiHazirla() {
    ...
    /* Eski tablolarda logo sütunu yoksa ekle (ALTER TABLE) */
    try {
        await sql`ALTER TABLE kurumlar ADD COLUMN IF NOT EXISTS logo TEXT NOT NULL DEFAULT ''`;
    } catch (e) { /* zaten varsa yok say */ }
    ...
    /* Fotoğraflar: kurum + TC'e göre base64 veri saklanır (robot foto yükleme) */
    await sql`
        CREATE TABLE IF NOT EXISTS fotograflar (
            ...
            veri        JSONB NOT NULL,
            guncelleme  TIMESTAMPTZ NOT NULL DEFAULT now(),
            PRIMARY KEY (kurum, tc)
        )`;
    try {
        await sql`ALTER TABLE fotograflar ADD COLUMN IF NOT EXISTS guncelleme TIMESTAMPTZ NOT NULL DEFAULT now()`;
    } catch (e) { /* zaten varsa yok say */ }
